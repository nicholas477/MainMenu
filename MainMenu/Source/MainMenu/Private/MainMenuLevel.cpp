...
TArray<TSoftObjectPtr<UMainMenuLevel>> UMainMenuLevel::GetMainMenuLevels()
{
	...
	{
		Maps.AddUnique(TSoftObjectPtr<UMainMenuLevel>(MapObject));
	}
...
TArray<UMainMenuLevel*> UMainMenuLevel::LoadMainMenuLevels()
{
	...
	for (const FSoftObjectPath& MapObject : MapObjects)
	{
		...
		{
			if (UMainMenuLevel* LoadedMapClass = LoadedBlueprint->GeneratedClass->GetDefaultObject<UMainMenuLevel>())
			{
		...
		{
			if (UMainMenuLevel* LoadedMapClass = LoadedBlueprintClass->GetDefaultObject<UMainMenuLevel>())
			{
				Maps.AddUnique(LoadedMapClass);
			}
		}
